...
**Coordinator hosts.** `--host hermes` names a coordinator agent that owns requirements and arbitration but has no CLI provider of its own. Coordinator defaults: reviewer = first provider, builder = first provider that is not the reviewer, inspector = `other_provider(builder)` — e.g. hermes plans, Claude reviews, Codex builds, and a **fresh Claude session** inspects Codex's changes; the builder must never inspect its own code. The inspect handoff carries the actual builder identity via `--builder`; the runner rejects an inspector that matches the builder and never infers authorship from the chosen inspector. `--provider`/`--builder` still accept real providers only; an explicit `--provider` on a review run reconciles the recorded reviewer/builder to who actually runs. Provider hosts (`claude`, `codex`) keep their pre-existing defaults.
