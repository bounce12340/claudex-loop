...
**Coordinator hosts.** `--host hermes` names a coordinator agent that owns requirements and arbitration but has no CLI provider of its own. Coordinator defaults: reviewer = first provider, builder = first provider that is not the reviewer, inspector = `other_provider(builder)` — e.g. hermes plans, Claude reviews, Codex builds, Claude must not inspect Codex's code (pass `--builder` on the inspect call, or the runner resolves the inspector from the effective provider). `--provider`/`--builder` still accept real providers only; an explicit `--provider` on a review or bare inspect reconciles the recorded roles to who actually runs. Provider hosts (`claude`, `codex`) keep their pre-existing defaults.
